// --------------------------------------------------------------------------------------------------------------------
// <copyright file="LocalDb.cs" company="">
//   
// </copyright>
// <summary>
...
namespace Fabric.Database.Testing.LocalDb
{
    ...
    public class LocalDb
    {
        ...
        /// </summary>
        public virtual string ConnectionString => this.BuildConnectionString(this.databaseName, this.dataSource);
        ...
        {
            var conn = new SqlConnection(this.BuildConnectionString("master", this.dataSource));
            conn.Open();
        ...
        public virtual bool CheckExists()
        {
            ...
            {
                cmd.CommandText = $@"
IF EXISTS(SELECT * FROM sys.databases WHERE name='{this.databaseName}')
BEGIN
    SELECT 1
END
ELSE
...
END",
        ...
        /// </summary>
        /// <param name="databaseName1">The name of the database to connect to. If null or empty string is provided, the connection string will connect to the master database.</param>
        /// <param name="dataSource1">The Sql Server instance to connect to.</param>
        /// <returns>A connection string</returns>
        protected virtual string BuildConnectionString(string databaseName1, string dataSource1)
        {
            var sb = new SqlConnectionStringBuilder { DataSource = dataSource1 };

            if (string.IsNullOrWhiteSpace(databaseName1) == false)
            {
